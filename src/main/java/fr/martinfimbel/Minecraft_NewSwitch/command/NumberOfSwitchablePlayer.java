...
public class NumberOfSwitchablePlayer extends AbstractLabelEdition<ISwitchConfiguration> {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		try {
			...
			if (number < 1) {
				sendSynchro(sender, ESwitchMessageCode.NUMBER_OF_SWITCHED_PLAYERS_INFERIOR_AT_ONE);
				return true;
			}
			get().setNumberOfPlayerSwitchable(Integer.parseInt(args[0]));
			sendSynchro(sender, ESwitchMessageCode.NUMBER_OF_SWITCHABLE_PLAYER_PER_TEAM_DEFINED, "" + get().getNumberOfPlayerSwitchable());
			return true;
		} catch (IndexOutOfBoundsException e) {
			sendSynchro(sender, ESwitchMessageCode.NUMBER_OF_SWITCHABLE_PLAYER_PER_TEAM_MISSING_NUMBER);
	...
	public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
		if (args.length == 1)
			return Arrays.asList(getMessage(sender, ESwitchMessageCode.NUMBER_OF_SWITCHABLE_PLAYER_ON_TAB_COMPLETE));
		return emptyList();
	}
}
