	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		try {
			...
			if (get().getPlayerDontReviveTime().equals(LocalTime.of(0, 0, 0)))
				sendSynchro(sender, ESwitchMessageCode.PLAYER_DONT_REVIVE_TIME__FROM_THE_BEGINNING);
			else
				sendSynchro(sender, ESwitchMessageCode.PLAYER_DONT_REVIVE_TIME__TIME_DEFINED, DisplayHelper.toString(get().getPlayerDontReviveTime(), false));
			return true;
		} catch (IndexOutOfBoundsException e) {
			sendSynchro(sender, ESwitchMessageCode.PLAYER_DONT_REVIVE_TIME__TIME_IS_MISSING);
			return false;
